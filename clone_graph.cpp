...
public:
    ...
    UndirectedGraphNode *cloneGraph(UndirectedGraphNode *node) {
        ...
        for(auto it = node->neighbors.begin(); it != node->neighbors.end(); it++){
            if((*it)->label == res->label){
                res->neighbors.push_back(res);
            }
            else if(save.find((*it)->label) != save.end()){
                res->neighbors.push_back(save[(*it)->label]);
            }
            else{
                res->neighbors.push_back(cloneGraph(*it));
            }
        }
        save[res->label] = res;
        return res;
